...
erg <- apply(matr,2,function(check){
      ...
      fuenfsechs   <- is.element(err,check) # err = c(5,6,7)
      fuenf        <- fuenfsechs[1] #this is true if there are 5s (so N=5)
      sechs        <- fuenfsechs[2]   #this is true if there are 6s (so "-"=6)

     # keine 5 oder 6  ni 5 ni 6, so not "-" or "N"
     if(!fuenf & !sechs){

       test <- unique(check)   #unicos numeros que toma check, que es cada columna, o sea position del cromosoma
       size <- length(test)    #el tamano del vector test. Si es una posicion con un unico nucleotido, sera 1, si no sera >1

       if(size==1){     #si estamos en el caso de una posicion donde no hay mas que un nucleotido
          # mono     <- 1
       return(RETURN)      #RETURN es un vector con 0s
       }

       if(size>2){       #si estamos hablando de posiciones mas polialelicas (mas de un alternate allele)
	 #en este caso tambien queremos que se marque como 1 en la matrix_pol
#	 x <- length(test)   #test puede ir de 1 a 6, pero aqui estamos desechando 5 y 6 que es N y "-"
	   #podemos mirar cual es el nt mas frecuente
#        RETURN[6] <- 1  #esto significa que es una biallelic position (aunque en realidad es polyallelic)
                        #pero queremos considerarla biallelic para que cuente para calcular Pi
#         lista <- list()
#	 for(i in 1:length(test)){
#		 lista[[i]] <- length(which(check==test[i]))
#	 }
#	 common <- which(unlist(lista)==max(unlist(lista)))
#	 commonnuc <- test[common]
#	 uncommon <- which(unlist(lista)==min(unlist(lista)))
#	 uncommonnuc <- test[uncommon[1]]
#	 RETURN[7] <- uncommonnuc
#       	 matrix_pol[check!=commonnuc] <- 1
#	 GLOBAL$GLOBALmatrix_pol <- cbind(GLOBAL$GLOBALmatrix_pol,matrix_pol)
      ...
      if(size==2){      #si tenemos una posicion bialelica (reference y alternate solo)
         ...
         nuc1       <- test[1]      #primer nucleotido (puede ser ref o alt, no nos importa)
         nuc2       <- test[2]	    #segundo nucleotido
         nuc12      <- c(nuc1,nuc2)  #combinacion de ambos
         # transition/transversion
         trans1        <- setequal(nuc12,pyrimid) # pyrimid      #esto va a estar mal calculado porque tenemos indels incluidos como nucleotidos
         trans2        <- setequal(nuc12,purin)   # purin        #same!
         ifelse(trans1 | trans2,RETURN[1] <- 1,RETURN[1] <- 0) # matrix_sv

         countnuc1  <- sum(check == nuc1)    #numero de muestras con pos==nuc1
         countnuc2  <- nsamtot - countnuc1   #numero de muestras con pos==nuc2

          if(countnuc1 <= countnuc2){
             RETURN[7]                 <- nuc1
             matrix_pol[check==nuc1]   <- 1   #en matrix_pol que es la matrix biallelica, con valores de 0 y 1, le corresponde a nuc1 el 1 por ser el menos frecuente
             GLOBAL$GLOBALmatrix_pol   <- cbind(GLOBAL$GLOBALmatrix_pol,matrix_pol)

          }else{
             RETURN[7]                 <- nuc2   #lo mismo pero si nuc2 es el alternate
...
#                 lista[[i]] <- length(which(check==hh[i]))
#         }
#         common <- which(unlist(lista)==max(unlist(lista)))
#         commonnuc <- hh[common]
#         uncommon <- which(unlist(lista)==min(unlist(lista)))
#         uncommonnuc <- hh[uncommon[1]]
#         RETURN[7] <- uncommonnuc
#         matrix_pol[check!=commonnuc&&check!=5] <- 1
#         matrix_pol[gapids] <- NaN #Gaps==N's
#	 GLOBAL$GLOBALmatrix_pol <- cbind(GLOBAL$GLOBALmatrix_pol,matrix_pol)
